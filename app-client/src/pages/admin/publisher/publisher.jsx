...
const PublisherPage = () => {
    ...
    const onDelete = async (id) => {
        const res = await deletePublisherAPI(id)
        if (res && res.data) {
    ...
    const columns = useMemo(
        () => [
            ...
            {
                title: common('module.publisher.name'),
                dataIndex: 'name',
                sorter: true,
            },
            {
                title: common('module.publisher.address'),
                dataIndex: 'address',
                sorter: true,
            },
            {
            ...
            },
            {
                ...
                render: (_, record) => (
                    <Space>
                        ...
                        <Popconfirm
                            title={common('module.publisher.delete')}
                            description={common(
                                'module.publisher.delete_confirm',
                                {
                                    name: record?.name,
                                }
                            )}
    ...
    return (
        <div style={{ background: '#fff', borderRadius: 8, padding: 16 }}>
            <Flex gap={8} style={{ marginBottom: 16 }} justify="space-between">
                <Input
                    placeholder={common('module.publisher.search')}
                    style={{ maxWidth: 320 }}
