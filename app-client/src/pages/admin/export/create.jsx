...
import { createExportAPI } from '../../../services/export.service.js'
...
const ExportCreatePage = () => {
    ...
    const onFinish = async (values) => {
        if (items.length === 0) {
            message.error(validation('export_list.empty'))
        }
        const bookExportItemRequests = items.map((item) => {
            return {
    ...
    const columns = [
        ...
        {
            title: common('module.export.total_price'),
        ...
        <div style={{ background: '#fff', borderRadius: 8, padding: 16 }}>
            ...
            >
                <Row gutter={24}>
                    <Col span={6}>
                        <Form.Item
                            label={common('module.export.id')}
                            name="id"
                            rules={[
                                {
                                    required: true,
                                    message: validation('export_id.required'),
                                },
                                {
                                    max: 20,
                                    message: validation('export_id.max', {
                                        max: 20,
                                    }),
                                },
                            ]}
                    ...
                    <Col span={6}>
                        ...
                        >
                            <Select
                                showSearch
                                placeholder={common(
                                    'module.export.supplier_placeholder'
                                )}
                    ...
                    <Col span={8}>
                        <Form.Item
                            label={common('module.export.note')}
                            name="note"
                            rules={[
                                {
                                    max: 200,
                    ...
                    <Col span={4}>
                        <Form.Item label={common('module.export.total_price')}>
                            <InputNumber
                                disabled
                                style={{ width: '100%' }}
                                formatter={(value) =>
