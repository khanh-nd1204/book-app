...
const ExportPDF = ({ books }) => {
    ...
    const generatePDF = () => {
        ...
        doc.setFontSize(16)
        doc.text(
            common('CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM'),
            pageWidth / 2,
            15,
            {
                align: 'center',
        ...
        doc.text(common('BIÊN BẢN NHẬP SÁCH'), pageWidth / 2, 45, {
            align: 'center',
        })

        doc.setFontSize(11)
        doc.text(`${common('Mã phiếu nhập')}: ${books?.id}`, 10, 60)
        ...
        doc.text(`${common('Lý do nhập')}: ${books?.note || ''}`, 10, 74)
        ...
        books?.bookImportItems?.forEach((item, index) => {
            tableRows.push([
                index + 1,
                item.book.title,
                item.book.isbn,
